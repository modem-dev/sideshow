...
test("publishes an issue-tree part; /s has no html doc for it", async () => {
  const app = makeApp();
  const root = {
    ref: "ENG-204",
    title: "Normalize issue states",
    state: "in-progress",
    source: "linear",
    children: [
      { ref: "#1432", title: "Dark mode regression", state: "done", source: "github" },
      {
        ref: "SUP-881",
        title: "SSO login fails",
        state: "blocked",
        source: "jira",
        children: [{ ref: "JS-4F2", title: "TypeError", state: "open", source: "sentry" }],
      },
    ],
  };
  const res = await app.request(
    "/api/surfaces",
    json({ title: "Sub-issues", parts: [{ kind: "issue-tree", root }] }),
  );
  assert.equal(res.status, 201);
  const surface = (await res.json()) as any;
  assert.deepEqual(surface.kinds, ["issue-tree"]);

  const full = (await (await app.request(`/api/surfaces/${surface.id}`)).json()) as any;
  assert.equal(full.parts[0].kind, "issue-tree");
  assert.deepEqual(full.parts[0].root, root);
  // issue-tree is viewer-rendered data, not a sandboxed html doc
  assert.equal((await app.request(`/s/${surface.id}?part=0`)).status, 404);
});

test("rejects an issue-tree with a bad state (strict REST) and a rootless one", async () => {
  const app = makeApp();
  const bad = await app.request(
    "/api/surfaces",
    json({
      parts: [{ kind: "issue-tree", root: { ref: "X", title: "x", state: "wat" } }],
    }),
  );
  assert.equal(bad.status, 400);
  const rootless = await app.request("/api/surfaces", json({ parts: [{ kind: "issue-tree" }] }));
  assert.equal(rootless.status, 400);
});

test("publish_surface MCP tool keeps a valid issue-tree and drops a rootless one", async () => {
  const app = makeApp();
  const published = (await (
    await app.request(
      "/mcp",
      mcpCall(2, "tools/call", {
        ...
        arguments: {
          ...
          parts: [
            ...
            {
              kind: "issue-tree",
              root: { ref: "ENG-204", title: "Epic", state: "open" },
            },
          ],
        },
      }),
  ...
  assert.equal(full.parts.length, 1);
  assert.equal(full.parts[0].kind, "issue-tree");
  assert.equal(full.parts[0].root.ref, "ENG-204");
});

// Build a `children`-chain of the given depth (1 = just the root).
function deepIssueTree(depth: number) {
  let node: any = { ref: "leaf", title: "leaf", state: "open" };
  for (let i = 0; i < depth - 1; i++) {
    node = { ref: `n${i}`, title: "t", state: "open", children: [node] };
  }
  return node;
}

test("strict REST rejects an over-deep issue-tree with 400, not a 500 crash", async () => {
  const app = makeApp();
  // ~5000 deep crashed the recursive parser before the depth guard; now a 400.
  const res = await app.request(
    "/api/surfaces",
    json({ parts: [{ kind: "issue-tree", root: deepIssueTree(200) }] }),
  );
  assert.equal(res.status, 400);
  assert.match(((await res.json()) as any).error, /exceeds .* levels deep|nodes/);
  // a tree at the limit still publishes
  const ok = await app.request(
    "/api/surfaces",
    json({ parts: [{ kind: "issue-tree", root: deepIssueTree(32) }] }),
  );
  assert.equal(ok.status, 201);
});

test("loose MCP path drops an over-deep issue-tree instead of crashing", async () => {
  const app = makeApp();
  const published = (await (
    await app.request(
      "/mcp",
      mcpCall(2, "tools/call", {
        ...
        arguments: {
          title: "Deep",
          parts: [
            { kind: "issue-tree", root: deepIssueTree(200) },
            { kind: "issue-tree", root: { ref: "ENG-1", title: "ok", state: "open" } },
          ],
        },
      }),
    )
  ...
  const payload = JSON.parse(published.result.content[0].text);
  const full = (await (await app.request(`/api/surfaces/${payload.id}`)).json()) as any;
  // the deep one is dropped; the valid one survives
  assert.equal(full.parts.length, 1);
  assert.equal(full.parts[0].root.ref, "ENG-1");
});

test("loose MCP path coerces an unknown issue state to 'open' and drops blank children", async () => {
  const app = makeApp();
  const published = (await (
    await app.request(
      "/mcp",
          ...
          parts: [
            {
              ...
              root: {
                ...
                state: "open",
                children: [
                  { ref: "#1", title: "real", state: "totally-bogus" },
                  {}, // junk: no ref/title -> dropped, not a phantom rollup row
                ],
              },
            },
          ],
  ...
  const payload = JSON.parse(published.result.content[0].text);
  const full = (await (await app.request(`/api/surfaces/${payload.id}`)).json()) as any;
  const kids = full.parts[0].root.children;
  assert.equal(kids.length, 1); // blank child dropped
  assert.equal(kids[0].ref, "#1");
  assert.equal(kids[0].state, "open"); // unknown state -> "open"
});

test("a surface can be updated to an issue-tree and its prior version reads back", async () => {
  const app = makeApp();
  const s = (await (
  ...
  const res = await app.request(`/api/surfaces/${s.id}`, {
    ...json({ parts: [{ kind: "issue-tree", root }] }),
    method: "PUT",
  });
  assert.equal(((await res.json()) as any).version, 2);

  const full = (await (await app.request(`/api/surfaces/${s.id}`)).json()) as any;
  assert.equal(full.parts[0].kind, "issue-tree");
  assert.deepEqual(full.parts[0].root, root); // nested shape round-trips through history archival
